...
export default class CodeStylerPlugin extends Plugin {
    ...
    async onload(): Promise<void> {
            ...
            (
                ...
                if (LANGUAGES[key]?.icon)
                    result[key] = URL.createObjectURL(
                        new Blob([`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32">${LANGUAGES[key].icon}</svg>`], {
                            type: "image/svg+xml",
                        })
                    );
                return result;
        ...
        this.addCommand({
            ...
            callback: () => {
                ...
                if (activeView) {
                    if (activeView.getMode() === "preview") readingDocumentFold(activeView.contentEl, true);
    ...
    async initialiseOnLayout(): Promise<void> {
        ...
        if (this.settings.externalReferenceUpdateOnLoad) await updateExternalReferencedFiles(this);
        else {
            await cleanExternalReferencedFiles(this);
    ...
    renderReadingView(): void {
        this.app.workspace.iterateRootLeaves((leaf: WorkspaceLeaf) => {
            if (leaf.view instanceof MarkdownView && leaf.view.getMode() === "preview") {
                ...
                if (plugins.isEnabled("execute-code")) {
                    //@ts-expect-error Undocumented Obsidian API
                    leaf.containerEl.querySelectorAll(".has-run-code-button").forEach((element: HTMLElement) => {
                        element.classList.remove("has-run-code-button");
                    });

                    plugins.getPlugin("execute-code")?.addRunButtons(leaf.view.contentEl, leaf.view.file?.path, leaf.view);
                }
            }
        });
    }
